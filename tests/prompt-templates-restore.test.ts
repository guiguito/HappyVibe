...
import { expandedHash, pairPromptTemplateItems, restoreItems, type RawMessage } from "../src/main/restore";
...
describe("pairPromptTemplateItems", () => {
  it("attaches the typed form to the user message whose text hashes to a logged invocation", () => {
    ...
    const paired = pairPromptTemplateItems(items, new Map([[expandedHash(EXPANDED), "/review src/foo.ts"]]));
    expect(paired[0]).toEqual({ kind: "user", text: EXPANDED, promptTemplate: { typed: "/review src/foo.ts" } });
  ...
  it("leaves a non-matching user message plain", () => {
    ...
    expect(pairPromptTemplateItems(items, new Map([[expandedHash(EXPANDED), "/review"]]))).toEqual([
  ...
  it("pairs by hash, not by position — a later invocation matches wherever it landed", () => {
    ...
    const paired = pairPromptTemplateItems(items, new Map([[expandedHash(EXPANDED), "/review"]]));
    expect(paired.map((i) => (i.kind === "user" ? i.promptTemplate?.typed ?? null : null))).toEqual([null, "/review"]);
  });
  ...
  it("is a no-op with no logged invocations", () => {
    ...
    expect(pairPromptTemplateItems(items, new Map())).toEqual([{ kind: "user", text: EXPANDED }]);
  });
});
