...
import { hashPromptTemplateFile, parsePromptTemplateFrontmatter, readPromptTemplateFile, scanPromptTemplatesDir } from "../src/main/promptTemplates/discovery";
...
describe("command discovery", () => {
  it("reads frontmatter and derives the name from the filename", () => {
    ...
    const [c] = scanPromptTemplatesDir(d, "managed");
  ...
  it("flags inline bash injection, which Pi does not support", () => {
    ...
    const byName = Object.fromEntries(scanPromptTemplatesDir(d, "managed").map((c) => [c.name, c]));
  ...
  it("does not recurse, and ignores non-markdown and dotfiles", () => {
    ...
    expect(scanPromptTemplatesDir(d, "managed").map((c) => c.name)).toEqual(["top"]);
  });
  ...
  it("hash changes when the file content changes", () => {
    ...
    const h1 = hashPromptTemplateFile(f);
    fs.writeFileSync(f, "two\n");
    expect(hashPromptTemplateFile(f)).not.toBe(h1);
  });

  it("returns [] for a missing root", () => {
    expect(scanPromptTemplatesDir("/nope/does/not/exist", "workspace")).toEqual([]);
  });

  // Below: the invariants the plan's cases lean on but don't assert directly.

  it("parsePromptTemplateFrontmatter reads only description/argument-hint, and strips the block from the body", () => {
    const parsed = parsePromptTemplateFrontmatter(`---\ndescription: "Does a thing."\nargument-hint: '[file]'\nallowed-tools: bash\n---\nBody line.\n`);
    ...
    expect(parsePromptTemplateFrontmatter("Just a body.\n").body).toBe("Just a body.\n");
  });

  it("readPromptTemplateFile keeps the body and sizes it — there is no per-turn card cost", () => {
    const d = tmp();
    const f = path.join(d, "x.md");
    fs.writeFileSync(f, `---\ndescription: Sized.\n---\n${"y".repeat(40)}\n`);
    const c = readPromptTemplateFile(f, "linked");
