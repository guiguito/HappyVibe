...
// Paths are built with node:path rather than written as POSIX literals, so these
// assertions mean the same thing on both platforms (PRD §4, Windows round). The
// module under test uses the host's path module against the real filesystem, so a
// hardcoded "/ws/sub/AGENTS.md" simply never matched the "C:\ws\sub\AGENTS.md" it
// builds — the walk-up was fine, the fixture was not.
const CWD = path.resolve("/ws");
/** An absolute path inside the fixture workspace. */
const P = (...seg: string[]): string => path.join(CWD, ...seg);
...
test("nearest file wins, walking up from the touched file's dir", () => {
  const exists = existsOf([P("sub", "AGENTS.md"), P("sub", "pkg", "AGENTS.md")]);
  expect(nearestAgentsMd(P("sub", "pkg", "deep", "f.ts"), CWD, exists)).toBe(path.join(P("sub", "pkg"), "AGENTS.md"));
  expect(nearestAgentsMd(P("sub", "f.ts"), CWD, exists)).toBe(path.join(P("sub"), "AGENTS.md"));
});

test("walks up multiple levels when intermediate dirs have no file", () => {
  const exists = existsOf([P("sub", "AGENTS.md")]);
  expect(nearestAgentsMd(P("sub", "a", "b", "c", "f.ts"), CWD, exists)).toBe(path.join(P("sub"), "AGENTS.md"));
});

test("stops at cwd EXCLUSIVE — the root AGENTS.md is never reported as nested", () => {
  const exists = existsOf([P("AGENTS.md")]);
  expect(nearestAgentsMd(P("sub", "f.ts"), CWD, exists)).toBeNull(); // only root exists → nothing nested
  expect(nearestAgentsMd(P("f.ts"), CWD, exists)).toBeNull(); // file directly in cwd → nothing nested
});

test("relative paths resolve against cwd", () => {
  const exists = existsOf([P("sub", "AGENTS.md")]);
  expect(nearestAgentsMd("sub/f.ts", CWD, exists)).toBe(path.join(P("sub"), "AGENTS.md"));
});
...
test("files outside cwd yield null (no injection for out-of-workspace paths)", () => {
  ...
  expect(nearestAgentsMd(path.resolve("/elsewhere/f.ts"), CWD, exists)).toBeNull();
  expect(nearestAgentsMd(P("..", "etc", "passwd"), CWD, exists)).toBeNull();
...
test("nestedFileList is path-sorted, cwd-relative, current-chars; deleted files drop out", () => {
  const files = new Set([P("z", "AGENTS.md"), P("a", "AGENTS.md"), P("gone", "AGENTS.md")]);
  const read = readOf({ [P("a", "AGENTS.md")]: "aaaa", [P("z", "AGENTS.md")]: "zz" });
  expect(nestedFileList(files, CWD, read)).toEqual([
    { dir: "a", path: P("a", "AGENTS.md"), chars: 4 },
    { dir: "z", path: P("z", "AGENTS.md"), chars: 2 },
  ]);
});
...
test("section contains delimited header, per-file relative dir + content", () => {
  const s = renderNestedSection([P("sub", "pkg", "AGENTS.md")], CWD, readOf({ [P("sub", "pkg", "AGENTS.md")]: "Use tabs." }));
...
test("empty set / all-deleted files render nothing", () => {
  ...
  expect(renderNestedSection([P("sub", "AGENTS.md")], CWD, readOf({}))).toBe("");
});

test("oversized files are capped with a truncation note", () => {
  const big = "x".repeat(NESTED_FILE_CAP + 100);
  const s = renderNestedSection([P("sub", "AGENTS.md")], CWD, readOf({ [P("sub", "AGENTS.md")]: big }));
...
test("Set semantics dedupe repeat discoveries (same subtree touched twice)", () => {
  ...
  set.add(P("sub", "AGENTS.md"));
  set.add(P("sub", "AGENTS.md"));
  const s = renderNestedSection(set, CWD, readOf({ [P("sub", "AGENTS.md")]: "once" }));
