...
import { applyPromptTemplatePair } from "../src/renderer/src/promptTemplatePair";
...
describe("applyPromptTemplatePair", () => {
  it("upgrades the idle path, where the bubble holds the TYPED text", () => {
    const out = applyPromptTemplatePair([user("hi"), user(PAIR.typed)], PAIR);
    expect(out[1]).toEqual({ kind: "user", text: PAIR.expanded, promptTemplate: { typed: PAIR.typed } });
  });

  it("upgrades the steered path, where the bubble holds the EXPANDED text", () => {
    const out = applyPromptTemplatePair([user("hi"), user(PAIR.expanded)], PAIR);
    expect(out[1]).toEqual({ kind: "user", text: PAIR.expanded, promptTemplate: { typed: PAIR.typed } });
  });

  it("both paths produce byte-identical items — the divergence this fixes", () => {
    const idle = applyPromptTemplatePair([user(PAIR.typed)], PAIR);
    const steered = applyPromptTemplatePair([user(PAIR.expanded)], PAIR);
    expect(idle).toEqual(steered);
  });

  it("decorates the most recent invocation, not the first", () => {
    const out = applyPromptTemplatePair([user(PAIR.typed), user("something else"), user(PAIR.typed)], PAIR);
    const cmd = (i: TranscriptItem): unknown => (i as { promptTemplate?: unknown }).promptTemplate;
  ...
  it("never re-decorates an item that already carries a command", () => {
    const already: TranscriptItem = { kind: "user", text: PAIR.expanded, promptTemplate: { typed: PAIR.typed } };
    const items = [already];
    // A duplicate notify must be a no-op — same array back, nothing re-wrapped.
    expect(applyPromptTemplatePair(items, PAIR)).toBe(items);
  });
 ...
 */
describe("applyPromptTemplatePair with @file mentions injected", () => {
  ...
  it("matches the optimistic bubble, which holds only what was typed", () => {
    const out = applyPromptTemplatePair([user(TYPED_CLEAN)], PAIR2);
    expect((out[0] as { promptTemplate?: { typed: string } }).promptTemplate).toEqual({ typed: TYPED_CLEAN });
  });

  it("titles the card with the typed command, never the injected file", () => {
    const out = applyPromptTemplatePair([user(TYPED_CLEAN)], PAIR2);
    const typed = (out[0] as { promptTemplate?: { typed: string } }).promptTemplate!.typed;
  ...
  it("still matches when the bubble holds main's injected form", () => {
    const out = applyPromptTemplatePair([user(INJECTED)], PAIR2);
    expect((out[0] as { promptTemplate?: { typed: string } }).promptTemplate).toEqual({ typed: TYPED_CLEAN });
  });
});
 ...
 */
describe("applyPromptTemplatePair when main rewrote the message", () => {
  ...
  it("still pairs when only the command name is common to both forms", () => {
    const out = applyPromptTemplatePair([user(COMPOSER)], PAIR3);
    expect((out[0] as { promptTemplate?: unknown }).promptTemplate).toBeDefined();
  });

  it("does not pair a different command that happens to be nearby", () => {
    const items = [user("/review something")];
    expect(applyPromptTemplatePair(items, PAIR3)).toBe(items);
  });
