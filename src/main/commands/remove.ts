...
export function planCommandRemoval(view: { id: string; source: string }): CommandDeletePlan {
  if (view.source === "bundled") {
    return {
      action: "refused",
      path: view.id,
      reason: "Bundled commands ship with HappyVibe and are reinstalled at startup — disable it instead.",
    };
  }
...
export function removeCommandFile(file: string, allowedRoots: string[]): void {
  ...
  if (!ok) throw new Error(`Refusing to delete ${path.resolve(file)}: outside the managed command roots.`);
