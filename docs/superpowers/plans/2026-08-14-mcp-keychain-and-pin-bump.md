...
| File | Responsibility |
|---|---|
| `pi-runtime/bin/mcp-oauth-bridge.mjs` | One-shot sidecar. Reads one JSON request on stdin, imports adapter auth code, writes one JSON response on stdout. Lives in `pi-runtime/` because that is where the adapter's `node_modules` is. |
...
describe("mcpAdapterStore", () => {
  ...
  it("reads back tokens it wrote, bound to the URL", async () => {
    const s = store();
    await s.writeTokens("hv-fixture", URL_A, { accessToken: "tok-xyz", refreshToken: "ref-1", expiresAt: 9999999999 });
    const same = await s.read([{ name: "hv-fixture", url: URL_A }]);
    expect(same["hv-fixture"]).toMatchObject({ status: "present", tokens: { accessToken: "tok-xyz" } });

    // The URL binding is the whole point: a different URL is not this credential.
    const other = await s.read([{ name: "hv-fixture", url: URL_B }]);
    expect(other["hv-fixture"].status).toBe("absent");
  });

  it("removes a credential outright", async () => {
    const s = store();
    await s.writeTokens("hv-fixture", URL_A, { accessToken: "tok-xyz" });
    await s.remove("hv-fixture");
    const out = await s.read([{ name: "hv-fixture", url: URL_A }]);
    expect(out["hv-fixture"].status).toBe("absent");
  });

  it("reads many servers in ONE spawn", async () => {
    const s = store();
    await s.writeTokens("a", URL_A, { accessToken: "a-tok" });
    await s.writeTokens("b", URL_A, { accessToken: "b-tok" });
    const out = await s.read([{ name: "a", url: URL_A }, { name: "b", url: URL_A }, { name: "c", url: URL_A }]);
    expect(Object.keys(out).sort()).toEqual(["a", "b", "c"]);
    expect(out.c.status).toBe("absent");
  });
});
```

Note: each `writeTokens` is its own spawn, so the memory store would be empty on the next one. The sidecar must therefore accept a **batch of ops in one request** and the wrapper must keep write-then-read pairs in a single call — see step 3's `ops` array. Adjust the test above only if step 3's protocol makes a write+read pair impossible; do not weaken the URL-binding assertion.
...
If Task 1 returned `SIDECAR_MODE = "bundled"`, add an esbuild step to `postinstall` that emits `pi-runtime/bin/mcp-oauth-bridge.mjs` from `oauth.ts` with `--external:@napi-rs/keyring`, and skip the two `import` lines below in favour of importing the bundle. Otherwise write it directly:
