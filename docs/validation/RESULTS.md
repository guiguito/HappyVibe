...
| V5 | Permission modal (GUI) | PASS (manual) | 2026-07-03 | User-validated in dev mode: modal appears for gated tool calls, Allow/Deny round-trip works end-to-end. V5-core (headless deny-blocks-tool-call with real model) already PASS (Task 6). Required a layout fix first (scaffold hero CSS clipped the whole UI — commit c54ba65). |
| V2 | Streaming chat | PASS (manual) | 2026-07-03 | User-validated in dev mode: setup auto-skipped via .env key, folder picker works, assistant text streams live. |
| V3 | Tool visibility | PASS (manual) | 2026-07-03 | User-validated in dev mode: tool-call cards render with status. Field names (`toolCallId`, `toolName`, `args`, `result`, `isError`) verified against Pi's `types.d.ts`. |
