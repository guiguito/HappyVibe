...
3. **$0.00 is never shown for an unknown price.** A call that burned tokens but priced at zero is labelled **unpriced**; the pill shows `$?` (or `+?` when only part of the total is unknown). This was a live defect, not a hypothetical: HappyVibe wrote custom endpoints to `models.json` with no `cost` block, so Pi defaulted every rate to zero and a real session reported 60,023 input tokens as free.
